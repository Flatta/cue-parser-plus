...
function parsePostgap(params, cuesheet) {
	...
	if (track.postgap) {
		throw new Error('Only one POSTGAP is allowed for one track.');
	}
...
function parsePregap(params, cuesheet) {
	...
	if (track.pregap) {
		throw new Error('Only one PREGAP is allowed for one track.');
	}
...
function parseRem(params, cuesheet) {
	...
	target.rems[params.shift()] = params.length > 0 ? params.join(' ') : null;
}
